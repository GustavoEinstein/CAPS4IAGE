...
const CatalogarProducoes = () => {
    ...
    if (mode === 'selecao') return <SelectionScreen onSelect={setMode} isMobile={isMobile} navigate={navigate} />;
    if (mode === 'manual') return <ManualFormSplit onBack={() => setMode('selecao')} navigate={navigate} isMobile={isMobile} />;
    if (mode === 'voz') return <VoiceFormV2 onBack={() => setMode('selecao')} navigate={navigate} isMobile={isMobile} />;
    return null;
};
            ...
            <div style={styles.containerCenter}>
                ...
                <div style={{...styles.selectionGrid, flexDirection: isMobile ? 'column' : 'row'}}>
                    <div style={styles.selectionCard} onClick={() => onSelect('manual')}>
                        <div style={styles.iconCircleBlue}><Keyboard size={32} color="#1565C0" /></div>
                        <h3 style={styles.cardTitle}>Preenchimento Manual</h3>
                        <p style={styles.cardDesc}>Preencha o formulário detalhado com duas colunas.</p>
                        <span style={styles.fakeLink}>Ir para formulário &rarr;</span>
                    </div>
                    <div style={styles.selectionCardAi} onClick={() => onSelect('voz')}>
                        <div style={styles.iconCirclePurple}><Wrench size={32} color="#7B1FA2" /></div>
...
const ManualFormSplit = ({ onBack, navigate, isMobile }) => {
    ...
    const [formData, setFormData] = useState({
        titulo: '', disciplina: storedDisc !== 'Outra' ? storedDisc : 'Geral', nivel: '', 
        modelo_ia: '', categoria: '', bncc: '', metodologia: '', duracao: '',
                    ...
                    <form onSubmit={handleSubmit}>
                        <div style={{...styles.splitLayout, flexDirection: isMobile ? 'column' : 'row'}}>
                            ...
                            <div style={{...styles.leftCol, width: isMobile ? '100%' : '35%'}}>
                                ...
                                <div style={styles.inputGroup}><label style={styles.label}><Layers size={14}/> Conteúdo Gerado</label><select name="categoria" value={formData.categoria} onChange={handleChange} style={styles.input} required><option value="">O que a IA ajudou a criar?</option><optgroup label="Planejamento"><option value="Plano de Aula">Plano de Aula</option><option value="Sequência Didática">Sequência Didática</option></optgroup><optgroup label="Recursos"><option value="Slide">Slide</option><option value="Quiz">Quiz</option><option value="Texto">Texto</option></optgroup><optgroup label="Prática"><option value="Simulação">Simulação</option><option value="Estudo de Caso">Estudo de Caso</option><option value="Prompt">Prompt</option></optgroup></select></div>
                            ...
                            <div style={{...styles.rightCol, width: isMobile ? '100%' : '65%'}}>
                                ...
                                <div style={styles.inputGroup}>
                                    <label style={styles.label}><Package size={14}/> Recursos (Clique para selecionar)</label>
                                    
                                    <div style={styles.resourcesGrid}>
                                        {RECURSOS_COMUNS.map(res => {
                                            const isSelected = formData.recursos.includes(res);
                                            return (
                                                <button key={res} type="button" onClick={() => toggleRecurso(res)} 
                                                    style={{...styles.resourceChip, ...(isSelected ? styles.resourceChipActive : {})}}>
                                                    {res}
...
const VoiceFormV2 = ({ onBack, navigate }) => {
    ...
    const toggleListening = () => { setIsListening(!isListening); if (!isListening) setTranscript("Ouvindo..."); };
    return (
        <div style={styles.container}>
            <div style={styles.card}>
                <button onClick={onBack} style={styles.backButton}><ArrowLeft size={20} /> Voltar para seleção</button>
                <div style={styles.voiceContainer}>
                    <div style={styles.voiceHeader}>
                        <div style={styles.iconCirclePurple}><Sparkles size={24} color="#7B1FA2" /></div>
                        <h2 style={{...styles.titleCenter, fontSize: '24px', color: '#4A148C'}}>Entrevista com a IA</h2>
                    </div>
                    <p style={{...styles.subtitleCenter, maxWidth: '600px', margin: '0 auto 20px auto'}}>Clique no microfone e descreva sua atividade.</p>
                    <div style={styles.micWrapper}>
                        <button onClick={toggleListening} style={{...styles.micButton, backgroundColor: isListening ? '#FFEBEE' : '#F3E5F5', borderColor: isListening ? '#EF5350' : '#E1BEE7', transform: isListening ? 'scale(1.1)' : 'scale(1)'}}>
                            <Mic size={48} color={isListening ? "#D32F2F" : "#7B1FA2"} />
                        </button>
                        <p style={styles.micStatus}>{isListening ? "Gravando..." : "Toque para falar"}</p>
                    </div>
                    <div style={styles.transcriptionBox}>{transcript ? <p>{transcript}</p> : <p style={{color: '#999'}}>Sua transcrição aparecerá aqui...</p>}</div>
                    <div style={styles.footerActions}><button style={styles.buttonCancel} onClick={onBack}>Cancelar</button><button style={{...styles.button, backgroundColor: '#7B1FA2'}}>Processar</button></div>
                </div>
            </div>
        </div>
    );
