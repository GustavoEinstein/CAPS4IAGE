...
const MainContent = () => {
    ...
    // --- INFO DO USUÁRIO ---
    const [headerInfo, setHeaderInfo] = useState({ firstName: '', disciplina: '' });
...
const Card = ({ data, isMobile, navigate }) => {
    ...
    const getTheme = (disciplina) => {
        ...
        const key = Object.keys(themes).find(k => disciplina?.includes(k)) || 'Default';
        return themes[key];
    };
...
const styles = {
    ...
    heroSection: { background: 'linear-gradient(135deg, #1565C0 0%, #1976D2 100%)', color: 'white', display: 'flex', flexDirection: 'column', justifyContent: 'center', boxShadow: '0 4px 20px rgba(21, 101, 192, 0.15)' },
    ...
    searchInput: { width: '100%', backgroundColor: '#FFFFFF', borderRadius: '50px', border: 'none', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', outline: 'none', color: '#333333', boxSizing: 'border-box', fontWeight: '500' },
    searchIcon: { position: 'absolute', top: '50%', transform: 'translateY(-50%)', color: '#1565C0' },
    contentArea: { width: '100%', boxSizing: 'border-box', display: 'flex', justifyContent: 'center' },
    mainCard: { width: '100%', maxWidth: '1700px', backgroundColor: '#ffffff', borderRadius: '16px', boxShadow: '0 8px 30px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', border: '1px solid #E0E0E0', boxSizing: 'border-box' },
    sectionHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', borderBottom: '1px solid #F0F2F5', paddingBottom: '15px' },
    ...
    grid: { display: 'grid', gap: '20px', border: 'none', backgroundColor: 'transparent' },
    ...
    cardHover: { transform: 'translateY(-5px)', boxShadow: '0 12px 20px rgba(0,0,0,0.1)', border: '1px solid #42A5F5' },
    ...
    cardTitle: { color: '#101828', margin: '0 0 6px 0', lineHeight: '1.4', fontWeight: '700', wordBreak: 'break-word' },
