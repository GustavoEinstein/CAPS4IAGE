...
const EditarProducao = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setIsDraftStatus(
          d.status === "Rascunho" || d.status === "Correção solicitada",
        )
  ...
  const toggleRecurso = (recurso) => {
    setFormData((prev) => {
      ...
      else return { ...prev, recursos: [...prev.recursos, recurso] }
    })
  }
  ...
  const addCustomResource = (e) => {
    if ((e.key === "Enter" || e.type === "click") && customResource.trim()) {
      ...
      if (!formData.recursos.includes(customResource.trim())) {
        setFormData((prev) => ({
          ...prev,
          recursos: [...prev.recursos, customResource.trim()],
        }))
      }
      setCustomResource("")
    }
  ...
  const handleUpdate = async (isDraft) => {
    if (
      ...
      Swal.fire(
        "Campos Incompletos",
        "Preencha os campos obrigatórios para enviar para revisão.",
        "warning",
      )
      return
    }
    ...
    dataToSend.append("link_material", formData.link_material)
        ...
        <div style={styles.topBar}>
          ...
          <div style={{ textAlign: isMobile ? "left" : "right" }}>
            ...
            <p style={styles.pageSubtitle}>
              {isDraftStatus
                ? "Termine de preencher seu rascunho."
                : "Faça as correções solicitadas e reenvie."}
            </p>
          </div>
        </div>
            ...
            >
              ...
              >
                <h3 style={styles.sectionTitle}>
                  <FileText size={20} color="var(--text-info)" /> Ficha Técnica
                </h3>

                <div style={styles.inputGroup}>
                  <label style={styles.label}>
                    Título <span style={styles.asterisk}>*</span>
                  </label>
                  <input
                    type="text"
                    name="titulo"
                ...
                <div style={styles.inputGroup}>
                  <label style={styles.label}>
                    Nível <span style={styles.asterisk}>*</span>
                  </label>
                  <select
                    name="nivel"
                    ...
                    <label htmlFor="file-upload" style={styles.uploadLabel}>
                      {formData.arquivo ? (
                        <div style={styles.fileSelected}>
                          <CheckCircle2 size={28} color="var(--text-success)" />
                          <span style={styles.fileName}>
                            Novo: {formData.arquivo.name}
                          </span>
                        </div>
                      ) : existingFile ? (
                        <div style={styles.fileSelected}>
                          <FileText size={28} color="var(--text-info)" />
                          <span style={styles.fileName}>
                            Manter atual (clique para trocar)
                          </span>
                        </div>
                      ) : (
                        <>
                          <div style={styles.uploadIconCircle}>
                            <UploadCloud size={20} color="var(--text-info)" />
                          </div>
                ...
                <div style={{ ...styles.inputGroup, marginTop: "15px" }}>
                  <label style={styles.label}>
                    <LinkIcon size={14} /> Link Externo (Opcional)
                  </label>
                  <input
                    type="url"
                    name="link_material"
              ...
              >
                <h3 style={styles.sectionTitle}>
                  <BookOpen size={20} color="var(--text-info)" /> Detalhamento
                  Pedagógico
                </h3>
                ...
                <div style={styles.inputGroup}>
                  ...
                  <textarea
                    ...
                    rows="2"
                    placeholder="Cite os códigos e objetivos da BNCC relacionados..."
                  />
                </div>
                  ...
                  <div style={styles.resourcesGrid}>
                    {RECURSOS_COMUNS.map((res) => (
                      <button
                        ...
                        style={{
                          ...styles.resourceChip,
                          ...(formData.recursos.includes(res)
                            ? styles.resourceChipActive
                            : {}),
                        }}
                      >
                        {res}
                ...
                <div style={styles.inputGroup}>
                  ...
                  <textarea
                    ...
                    placeholder="Descreva como foi a aplicação em sala de aula..."
                  />
                </div>

                <div style={styles.inputGroup}>
                  <label style={styles.label}>
                    <Target size={14} /> Resultados / Evidências
                  </label>
                  <textarea
                    name="resultados"
                    ...
                    style={styles.textarea}
                    rows="2"
                    placeholder="O que os alunos produziram ou demonstraram?"
                  />
                </div>
...
const styles = {
  ...
  asterisk: { color: "var(--text-danger)", marginLeft: "2px" },
  input: {
    width: "100%",
    padding: "12px 15px",
    borderRadius: "8px",
