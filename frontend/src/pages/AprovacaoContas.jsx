  ...
  const carregarUsuarios = async () => {
    try {
      ...
      if (error.response?.status !== 401 && error.response?.status !== 403) {
        Swal.fire(
          "Erro!",
          "Erro ao carregar lista de aprovação. Verifique sua conexão.",
          "error",
        )
      }
    } finally {
  ...
  if (loading) {
    return (
      <div style={styles.loadingContainer}>
        ...
        <p style={{ color: "#546E7A", marginTop: "10px" }}>
...
const styles = {
  ...
  // --- ESTILOS COMUNS E DO MOBILE CARD ---
  mobileCard: {
    backgroundColor: "white",
    borderRadius: "12px",
    padding: "20px",
  ...
  mobileDate: { fontSize: "12px", color: "#94A3B8" },
